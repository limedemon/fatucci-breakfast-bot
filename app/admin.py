...
TEXT_TITLES: dict[str, str] = {
    ...
    "choose_object": "Выбор апартаментов на старте",
    "request_object": "Запрос новых апартаментов",
    "request_sent": "Ответ на заявку об апартаментах",
...
async def _ensure_admin_button(ev: Event, ch: Channel) -> None:
    """Закрепить кнопки «Админ-панель» и «Поддержка» — один раз на чат."""
    if str(ev.chat_id) != str(ev.user_id):
        return                                   # в рабочем чате кнопки не нужны
    await ch.show_admin_button(
        str(ev.chat_id),
        "🛠 Внизу закреплены <b>Админ-панель</b> и <b>Поддержка</b> — "
        "открывайте их оттуда в любой момент.",
    )
...
async def _check_orders_chat(chat_id: str) -> str:
    ...
    if not chat_id:
        return ("ℹ️ Чат не задан — заказы приходят лично администраторам.\n"
                "Чтобы завести рабочий чат, отправьте <code>/clip</code> в группе с ботом.")
    ...
    return (
        ...
        "Проще всего: добавьте бота в группу менеджеров, дайте ему право писать "
        "и отправьте там команду <code>/clip</code> — чат привяжется сам. "
        "Команда <code>/id</code> просто покажет ID, если хотите вписать вручную.\n\n"
