...
def price_for(obj: Row, breakfast: Optional[Row] = None) -> int:
    """Цена одного сета для этого дома.

    У объекта «общий QR» цена фиксированная и главнее всего: по нему едут
    заказы с адресов вне списка, и они должны стоить одинаково, какой бы сет
    ни выпал в этот день.

    У обычного дома наоборот: своя цена сета важнее цены дома — так дорогой
    сет стоит дороже везде.
    """
    if obj["is_general"] and obj["price_kop"]:
        return int(obj["price_kop"])
