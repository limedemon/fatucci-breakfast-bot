...
def price_for(obj: Row, breakfast: Optional[Row] = None) -> int:
    """Цена одного сета: своя цена сета важнее цены объекта.

    Заказы по адресам вне списка привязаны к объекту «общий QR», поэтому
    их цена — это его цена. Отдельной настройки нет специально: одна цена
    в одном месте, а не два поля, которые спорят друг с другом.
    """
